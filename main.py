...
def scaleWindows(image, (x, y, w, h), scale, iterations):
	...
	for k in range(1, iterations + 1):
		...
		if 0 < int(scaledY) < image.shape[0] and 0 < int(scaledY + scaledH) < image.shape[0] and 0 < int(scaledX) < \
				image.shape[1] and 0 < int(scaledX + scaledW) < image.shape[1]:
			print 'yielding'
		...
		if index >= startIndex + 3:
			...
			for k in range(8):
				if list1[k] is not None and list2[k] is not None and list3[k] is not None and list4[k] is not None:
					...
					flag = 1
					print image1[list1[k][1]:list1[k][1] + addImage.shape[0],
						  list1[k][0] - addImage.shape[1]:list1[k][0]].shape
